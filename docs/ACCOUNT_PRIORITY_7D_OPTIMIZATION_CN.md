...
通过管理员接口定时查询账号 5h / 7day 用量，并动态调整账号 `priority` / `load_factor`，让账号在 7day 刷新前尽量接近满用，同时避免单个账号被快速打满。
...
- 不因为补量策略显著增加 429
- 不让少数账号长期承担大部分流量
- 不侵入 sub2api 请求级调度逻辑，只通过 Admin API 更新 `priority` / `load_factor`
...
- 不直接指定每个请求使用哪个账号
- 不把 `priority` 当成精确流量权重；补量权重使用 `load_factor`
...
方向仍正确（数值小者得分高），但不再是硬隔离——低 priority 档不能吸走全部新会话，
...
源码核实：账号的 `EffectiveLoadFactor()` 只用于构造
`GetAccountsLoadBatch` 的 `MaxConcurrency`，进而计算：

```text
LoadRate = (current_concurrency + waiting_count) * 100 / EffectiveLoadFactor
```

真实抢占账号并发槽位时仍使用账号原始 `concurrency`，不是
`load_factor`。因此 `load_factor` 更像**调度权重/容量权重**：
在同一 `priority` 档内，把落后账号的 `load_factor` 调大，会让它在同等
并发下呈现更低 `LoadRate`，从而拿到更多新请求；但不会突破账号自己的实际
并发上限，也不会像单独低 `priority` 那样独占入口。
...
```text
外部控制器用 priority 做保护/兜底，用 load_factor 做补量权重
sub2api 原有调度负责同 priority 内的负载、LRU 分散
...
## priority 保护档与 load_factor 权重

采用有限 priority 档位，主要用于正常 / 保护 / 兜底。由于 priority 是严格
排队顺序（相邻档位仅在高档全部不可用时才有区别），不能再把补量账号单独
放进更小 priority，否则会形成唯一入口。
...
| priority | 含义 | 说明 |
|---:|---|---|
| 1010 | 保留 | 不作为默认补量档，兼容旧配置 / 手工回滚 |
| 1030 | 保留 | 不作为默认补量档，兼容旧配置 / 手工回滚 |
| 1050 | 正常 | 默认节奏；补量账号也保持此 priority |
| 1070 | 超前保护 | 减少新会话分配 |
| 1099 | 兜底保护 | 接近上限，不应继续吸流（注意：存量粘性流量仍会继续） |

补量使用 `load_factor`：

| reason | target priority | target load_factor |
|---|---:|---:|
| `boost` | 1050 | `ceil(concurrency * boost_load_factor_multiplier)` |
| `mild_boost` | 1050 | `ceil(concurrency * mild_load_factor_multiplier)` |
| `normal` / 保护类原因 | 1050 / 1070 / 1099 | `concurrency` |

所有目标 `load_factor` 受 `max_load_factor` 上限约束。控制器每轮会把 normal /
protect 账号的 `load_factor` 拉回当前 `concurrency`，避免旧 boost 权重残留。
...
if five_hour_used >= 92:
    不提高 load_factor（不主动吸流）

if five_hour_used >= 98:
    priority = 1099，load_factor = concurrency（兜底，与请求级保护冗余）
```
...
if seven_day_used >= 99.2:
    ...
    priority = 1070，load_factor = concurrency
...
例如有 40 个可调度账号，则每小时最多 6 个账号进入 `boost`
（默认 `load_factor = concurrency * 3`）。其它轻微落后账号最多进入
`mild_boost`（默认 `load_factor = concurrency * 2`）。

由于这些账号仍保持同一个 `priority=1050`，sub2api 会继续在同档内按
`LoadRate` / LRU 分散，不会出现单个账号因为独占最低 priority 被冲烂。

## 防抖策略

默认每小时最多把 priority 往高流量方向移动一个档位：
...
2. **窗口尾部紧急补量**：`remaining_hours < 12` 且明显落后时，
   允许直接回到 `1050`，并通过 `load_factor` 补量
...
for a in eligible:
    ...
    if seven_day >= 99.2 or a.five_hour_used >= 98:
        target_priority[a] = 1099; target_load_factor[a] = concurrency[a]; continue
    if seven_day >= 97:
        target_priority[a] = 1070; target_load_factor[a] = concurrency[a]; continue
    if a.cooldown_until > now:
        target_priority[a] = max(current_priority[a], 1070)
        target_load_factor[a] = concurrency[a]; continue
    if recent_hour_burn(a) > safe_hour_cap(a) and a 没有明显落后:
        target_priority[a] = 1070; target_load_factor[a] = concurrency[a]
        a.cooldown_until = now + 1h; continue
    if 首次接管且 priority 不在受控档位:
        target_priority[a] = 1050; target_load_factor[a] = concurrency[a]; continue
...
for a in remaining:
    if a in strong 名额:
        target_priority[a] = 1050
        target_load_factor[a] = min(max_load_factor, ceil(concurrency[a] * 3))
    else if a in mild 名额:
        target_priority[a] = 1050
        target_load_factor[a] = min(max_load_factor, ceil(concurrency[a] * 2))
    else:
        target_priority[a] = 1050
        target_load_factor[a] = concurrency[a]

    # 防抖只作用于 priority；load_factor 可在 normal 档内直接调整
    if not (remaining_hours(a) < 12 and target_priority[a] < current_priority[a]):
        target_priority[a] = one_band_step(current_priority[a], target_priority[a])

# 第四步：按 (priority, load_factor) 批量更新
for (priority, load_factor), ids in group_changed_accounts():
    if ids:
        POST /api/v1/admin/accounts/bulk-update
          {account_ids: ids, priority: priority, load_factor: load_factor}
```
...
| 批量更新档位 / 权重 | `POST /api/v1/admin/accounts/bulk-update` |
...
`priority` / `load_factor` 为指针语义的局部更新，不会覆盖其它字段（单账号
...
- 高 `load_factor` 账号数量稳定受控，没有长期热点账号
- 5h 触顶、429 没有明显增加
